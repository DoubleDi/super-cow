...
toggleGameHardness :: Universe -- ^ Вселенная
                   -> Int      -- ^ Коэфицент усложнения
                   -> Universe
...
updateBackground dt background = background
  { mapBackgroundPicture = updateBackgrounds dt 
    (mapBackgroundPicture background) (backgroundPictureSpeed background)
  }
...
updateBackgrounds dt backgrounds speed =
  dropWhile (\o -> fst (backgroundPicturePosition o) < screenLeft)
    (map (\o -> o 
      { backgroundPicturePosition = (coordX o - dx, coordY o)
      }) backgrounds
    )
  where
...
updateLife _ u
  ...
  | collisionMulti cow (mapGoodBirds obstacleMap) = u
    ...
    , universeMap  = collisionHandle obstacleMap cow (cowBonus cow)
    , universeCow  = cow 
      { cowBonus = tryAddInvincibleBonus (cowBonus cow) (life - 1) 
      }
    }
  | collisionMulti cow (mapBadBirds obstacleMap)  = u
    ...
    , universeMap  = collisionHandle obstacleMap cow (cowBonus cow)
    , universeCow  = cow 
      { cowBonus = tryAddInvincibleBonus (cowBonus cow) (life - 2) 
      }
    }
  ...
  | collisionMulti cow (mapBonusItems obstacleMap) = u
    { universeMap  = collisionHandle obstacleMap cow (cowBonus cow)
    , universeCow  = cow 
      { cowBonus = initBonus (bonusItemType 
        (getCollisionObstacle cow (mapBonusItems obstacleMap))) u
      }
    }
  | otherwise                                      = u
  where
